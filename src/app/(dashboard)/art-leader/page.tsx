...
export default async function ArtLeaderPage() {
    const session = await getServerSession(authOptions);
    if (!session || session.user.role !== "ART_LEADER") redirect("/login");
    const plans = await getActionPlans();
    return <TeamRoleDashboardView role="ART_LEADER" plans={plans as any[]} />;
}
