...
export function PlanApprovalHeader({ planId, status, canApprove = false }: { planId: string, status: string, canApprove?: boolean }) {
    ...
    if (status === "APPROVED") {
        return (
            <div className={`bg-emerald-500/10 border border-emerald-500/20 p-6 rounded-3xl flex items-center justify-between mb-8 shadow-sm ${isRtl ? 'flex-row-reverse' : ''}`}>
                <div className={`flex items-center gap-4 ${isRtl ? 'flex-row-reverse' : ''}`}>
                    <div className="p-3 bg-emerald-500 rounded-2xl text-white shadow-lg shadow-emerald-200">
                        <CheckCircle2 className="h-6 w-6" />
                    </div>
                    <div className={isRtl ? 'text-right' : 'text-left'}>
                        <h3 className="font-black text-xl text-emerald-900 dark:text-emerald-400 tracking-tight">
    ...
    return (
        <div className={`bg-primary/5 border border-primary/10 p-6 rounded-3xl flex flex-col md:flex-row items-center justify-between gap-6 mb-8 group hover:bg-primary/10 transition-all duration-300 ${isRtl ? 'md:flex-row-reverse' : ''}`}>
            ...
            {canApprove && (
                <Dialog open={open} onOpenChange={setOpen}>
                    ...
                    <DialogContent className="rounded-3xl" dir={isRtl ? "rtl" : "ltr"}>
