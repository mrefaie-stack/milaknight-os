...
const IMPACT_LABELS_AR = { HIGH: "تأثير عالي", MEDIUM: "تأثير متوسط", LOW: "تأثير منخفض" };
const IMPACT_LABELS_EN = { HIGH: "High Impact", MEDIUM: "Medium Impact", LOW: "Low Impact" };

export function IndustryView({ items, updatedAt }: { items: IndustryItem[]; updatedAt: Date | null }) {
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h1 className="text-4xl font-black tracking-tighter premium-gradient-text uppercase">
                                {isRtl ? "أخبار الصناعة" : "Industry Updates"}
                            </h1>
                            <p className="text-sm text-muted-foreground font-medium opacity-70">
                                {isRtl ? "آخر المستجدات في قطاعك" : "Latest news tailored to your sector"}
                            </p>
                        </div>
                    </div>
                </div>
                                ...
                                <div className={`flex items-start justify-between gap-3 ${isRtl ? 'flex-row-reverse' : ''}`}>
                                    ...
                                    <div className={`flex gap-2 flex-wrap ${isRtl ? 'flex-row-reverse' : ''}`}>
                                        ...
                                        <Badge variant="outline" className={`text-[9px] font-black uppercase tracking-widest rounded-full ${IMPACT_STYLES[item.impact] || IMPACT_STYLES.MEDIUM}`}>
                                            {isRtl ? IMPACT_LABELS_AR[item.impact] : IMPACT_LABELS_EN[item.impact]}
                                        </Badge>
                                    </div>
                                </div>
