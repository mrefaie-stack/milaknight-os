...
export function ReportCreatorClient({ clients, initialData }: { clients: any[], initialData?: any }) {
    ...
    // Platforms that were auto-fetched (from generateAutoReport)
    const autoFetchedPlatforms: string[] = (() => {
        try {
            const raw = initialData?.metrics
                ? (typeof initialData.metrics === 'string' ? JSON.parse(initialData.metrics) : initialData.metrics)
                : null;
            return raw?._autoGenerated?.sourcePlatforms || [];
        } catch { return []; }
    })();
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-8 max-w-6xl mx-auto pb-24">
            ...
            {autoFetchedPlatforms.length > 0 && (
                <div className={`flex items-center gap-3 rounded-xl border border-primary/20 bg-primary/5 px-4 py-3 ${isRtl ? 'flex-row-reverse text-right' : ''}`}>
                    <Zap className="h-5 w-5 text-primary shrink-0" />
                    <div>
                        <p className="text-sm font-semibold text-primary">
                            {isRtl ? 'تقرير تلقائي' : 'Auto-Generated Report'}
                        </p>
                        <p className="text-xs text-muted-foreground">
                            {isRtl
                                ? `تم جلب بيانات ${autoFetchedPlatforms.map(p => p === 'facebook' ? 'فيسبوك' : p === 'instagram' ? 'إنستجرام' : p).join(' و')} تلقائياً. راجع الأرقام وأضف المنصات المتبقية يدوياً.`
                                : `Data auto-fetched for ${autoFetchedPlatforms.join(', ')}. Review the numbers and fill in remaining platforms manually.`}
                        </p>
                    </div>
                </div>
            )}
                        ...
                        <div className={`flex items-center gap-4 border-b-2 border-border pb-5 ${isRtl ? 'flex-row-reverse' : ''}`}>
                            ...
                            <div className={`flex-1 ${isRtl ? 'text-right' : ''}`}>
                                <div className={`flex items-center gap-2 ${isRtl ? 'flex-row-reverse justify-end' : ''}`}>
                                    ...
                                    {autoFetchedPlatforms.includes(p.id) && (
