...
import {
    ...
    Search, Video, ExternalLink, RefreshCw, CalendarCheck
} from "lucide-react";
...
function formatEventTime(start: string, end: string) {
    ...
    const dayLabel = isToday ? "Today" : isTomorrow ? "Tomorrow" : s.toLocaleDateString("en", { weekday: "long", month: "short", day: "numeric" });
...
function ScheduleModal({ meetingId, onClose, onScheduled }: { meetingId: string; onClose: () => void; onScheduled: (id: string, scheduledAt: string, meetLink: string | null) => void }) {
    const [value, setValue] = useState("");
    const [loading, setLoading] = useState(false);

    // Set default to nearest future hour
    useEffect(() => {
        const d = new Date();
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm px-4" onClick={onClose}>
            <div className="w-full max-w-sm bg-card border border-white/10 rounded-3xl p-6 space-y-5 shadow-2xl" onClick={e => e.stopPropagation()}>
                <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-1">Set date & time — Google Meet link will be auto-generated if you're signed in with Google</p>
                </div>
                <input
                    type="datetime-local"
                    value={value}
...
export function MeetingRequestsUI({
    ...
    const isStaff = userRole === "AM" || userRole === "ADMIN";
                                    ...
                                    <CardContent className="p-0">
                                        <div className={cn("flex flex-col md:flex-row md:items-center p-6 gap-6", isRtl ? "md:flex-row-reverse" : "")}>
                                            ...
                                            <div className={cn("flex flex-col md:items-end gap-3 min-w-[150px]", isRtl ? "md:items-start" : "")}>
                                                ...
                                                {/* Client: show Meet link button */}
                                                {userRole === "CLIENT" && req.status === "SCHEDULED" && req.meetLink && (
                                                    <a
                                                        href={req.meetLink}
                                                        target="_blank"
                                                        rel="noopener noreferrer"
                ...
                <section className="space-y-5">
                    ...
                    {!calConnected ? (
                        <div className="rounded-2xl border border-white/8 bg-white/3 p-8 flex flex-col items-center gap-4 text-center">
                            ...
                            <p className="text-xs text-muted-foreground">{isRtl ? "سجّل دخول بجوجل لتشاهد الاجتماعات" : "Sign in with Google to see your calendar events"}</p>
                            <button
