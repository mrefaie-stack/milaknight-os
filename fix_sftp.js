...
// Write fixed file locally first
...
async function main() {
    ...
    for (const conn of connections) {
        ...
        try {
            ...
            console.error(\`Error testing \${conn.platform}:\`, (e as Error).message);
        }
    }
...
const tmpFile = path.join(__dirname, '_api_test_fixed.ts');
fs.writeFileSync(tmpFile, fixedContent);

const conn = new Client();
conn.on('ready', () => {
  console.log('Connected. Uploading fixed api-test.ts via SFTP...');
  conn.sftp((err, sftp) => {
    if (err) throw err;
    sftp.fastPut(tmpFile, '/root/milaknight-os/api-test.ts', (err2) => {
      if (err2) {
